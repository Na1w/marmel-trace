#!/usr/bin/env python3
"""
Photorealistic Space Scene Generation for Marmel Raytracer:
- Earth: Multi-layered physical planetary system:
  1. Base deep oceanic sphere with realistic Cook-Torrance Fresnel specular water reflection.
  2. Suspended semi-transmissive cloud envelope catching diffuse sunlight.
  3. Concentric Rayleigh scattering atmosphere shell producing an authentic glowing cyan limb.
- Sun: Balanced incandescent golden star with natural solar corona.
- Transiting Moon: Celestial eclipse silhouette with basaltic regolith material.
- Distant Gas Giant: Atmospheric latitudinal cloud bands with an elegant, thin planetary ring system.
- Deep Space: Pristine velvet black cosmos with astronomical diamond pinpoint stars.
"""
...
""")
    ...
    # Balanced, elegant solar radiation - rich golden corona
    lines.append(f"""sky {{
...
""")
    ...
    materials = [
        # --- Sun Core: Incandescent golden solar furnace ---
        ("mat_sun_core", "pbr = 1\n  albedo = 0.1 0.1 0.1\n  emissive = 3.6 2.4 0.38\n  roughness = 0.5"),

        # --- Earth Layer 1: Ocean Body (Deep marine blue with Fresnel water specular glint) ---
        ("mat_earth_ocean", "pbr = 1\n  albedo = 0.10 0.20 0.38\n  roughness = 0.18\n  metallic = 0.03"),

        # --- Earth Layer 2: Cloud Envelope (Semi-transmissive white weather formations) ---
        ("mat_earth_clouds", "pbr = 0\n  albedo = 0.88 0.90 0.95\n  specular = 0.25 0.25 0.30\n  shininess = 32\n  transparency = 0.75\n  texture_scale = 1.6\n  texture_color_a = 0.95 0.95 0.98\n  texture_color_b = 0.25 0.30 0.40"),

        # --- Earth Layer 3: Concentric Rayleigh Scattering Atmosphere ---
        ...
        ("mat_gas_giant", "pbr = 1\n  albedo = 0.65 0.60 0.52\n  roughness = 0.52\n  metallic = 0.0\n  texture_scale = 0.75\n  texture_color_a = 0.82 0.76 0.64\n  texture_color_b = 0.48 0.40 0.30"),

        # --- Gas Giant Rings: Dusty crystalline ice/rock rings ---
        ("mat_rings", "pbr = 1\n  albedo = 0.70 0.65 0.55\n  roughness = 0.40\n  metallic = 0.05"),

        # --- Astronomical Diamond Stars ---
  ...
  material = mat_moon
    ...
    # 3. Distant Companion Gas Giant with Planetary Rings
  ...
  material = mat_gas_giant
    ...
    # Gas giant rings: thin tilted disk/cylinder system
    # Tilted ring cylinder slice centered on gas giant
    ring_normal = norm((0.25, 1.0, 0.35))
    ring_half_thick = 0.008
    ring_base = sub(upper_pos, scale(ring_normal, ring_half_thick))
    ring_top  = add(upper_pos, scale(ring_normal, ring_half_thick))
  ...
  material = mat_earth_ocean
    ...
    # Layer 2: Suspended cloud envelope
    prims.append(f"""sphere {{
  center   = {hero_center[0]:.3f} {hero_center[1]:.3f} {hero_center[2]:.3f}
  ...
  material = mat_earth_clouds
    ...
    # Layer 3: Rayleigh atmosphere shell
    prims.append(f"""sphere {{
  center   = {hero_center[0]:.3f} {hero_center[1]:.3f} {hero_center[2]:.3f}
