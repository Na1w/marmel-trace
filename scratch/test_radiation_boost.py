...
""")
    ...
    # Sun positioned to the right
    sun_pos = (-8.2, 0.35, 11.5)
    sun_dir = norm(sub(sun_pos, cam_eye))

    # Powerful, blazing golden-orange solar radiation inspired by preview 4:
    # Rich warm color, stronger glow (strength ~ 2.8), broader coronal flare (exp ~ 45.0)
    lines.append(f"""sky {{
  sun_dir            = {sun_dir[0]:.4f} {sun_dir[1]:.4f} {sun_dir[2]:.4f}
  sun_color          = 1.00 0.72 0.16
  sun_radius         = 0.0
  horizon_color      = 0.0 0.0 0.0
  zenith_color       = 0.0 0.0 0.0
  gradient_gamma     = 1.0
  sun_glow_exponent  = 48.0
  sun_glow_strength  = 2.8
  cloud_coverage     = 2.0
}}
""")

    materials = [
        # Sun core - intense incandescent solar furnace
        ("mat_sun_core", "pbr = 1\n  albedo = 0.1 0.1 0.1\n  emissive = 5.2 3.6 0.55\n  roughness = 0.5"),
